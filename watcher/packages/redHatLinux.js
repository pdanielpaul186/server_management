...
watch.createMonitor('./Output/RedHatLinux',(monitor)=>{
    monitor.on('created',(path,stat)=>{
        ...
        for(var i=0;i<=arr.length-1;i++){
            ...
            console.log(preHash);
            mongo.collection('RedHatLinux').find({hash:preHash}).count((err,data)=>{
                if(err){
                    console.log(err)
                }
                else{
                    if(data==0){
                        ...
                        mongo.collection('RedHatLinux').insertOne(json,(err,data)=>{
                            if(err){
...
console.log("WATCHING FOR FILES IN RED HAT LINUX FOLDER !!!!!!!")