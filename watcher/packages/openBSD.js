...
watch.createMonitor('./Output/OpenBSD',(monitor)=>{
    monitor.on('created',(path,stat)=>{
        ...
        for(var i=0;i<=arr.length-1;i++){
            ...
            console.log(preHash);
            mongo.collection('OpenBSD').find({hash:preHash}).count((err,data)=>{
                if(err){
                    console.log(err)
                }
                else{
                    if(data==0){
                        ...
                        mongo.collection('OpenBSD').insertOne(json,(err,data)=>{
                            if(err){
...
console.log("WATCHING FOR FILES IN OPEN BSD LINUX FOLDER !!!!!!!")