...
watch.createMonitor('./Output/SUSELinux',(monitor)=>{
    monitor.on('created',(path,stat)=>{
        ...
        for(var i=0;i<=arr.length-1;i++){
            ...
            console.log(preHash);
            mongo.collection('SUSELinux').find({hash:preHash}).count((err,data)=>{
                if(err){
                    console.log(err)
                }
                else{
                    if(data==0){
                        ...
                        mongo.collection('SUSELinux').insertOne(json,(err,data)=>{
                            if(err){
...
console.log("WATCHING FOR FILES IN SUSE LINUX FOLDER !!!!!!!")